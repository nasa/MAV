...
public class DepartureTimeBasedSchedule extends AbstractExternalActivity {

	public final static Logger LOGGER = Logger.getLogger(DepartureTimeBasedSchedule.class);
	...
	public void doActivity() throws ExternalException {
		...
		synchronized (Utils.getInstance()) {
			...
			List<AFO> tssTimeLine = new ArrayList<AFO>();
			Map<AFO, IAgent> slotMarkers = new HashMap<AFO, IAgent>();

			Map<Object, Object> TSSTimeLine = Utils.readSomeTimeLineMapData(i_TSS, ctx, slotMarkers, tssTimeLine,
					"tssTimeLine");

			System.out.print("DepartureTimeBasedSchedule::doActivity([ ");
			...
			try {
				...
				tssTimeLine = BrahmsWrapper.invokeDSAS1(this.getClass().getName(),tssTimeLine, globalClockTime);
			} catch (Exception e) {
				e.printStackTrace();
				System.exit(1);
			}
